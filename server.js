...
app.get('/api/public-list', async (req, res) => {
  ...
  if (error) return res.status(500).json({ error: 'Erè sèvè.' });
  res.json({ page, pageSize, total: count, list: data });
});
...
app.get('/api/admin/list', verifyAdmin, async (req, res) => {
  ...
  let q = supabase
    .from('registrations')
    .select('id, name, email, country_name, full_number, created_at', { count: 'exact' })
    .order('created_at', { ascending: false })
    .range(from, to);
...
app.get('/api/admin/export-csv', verifyAdmin, async (req, res) => {
  const { data, error } = await supabase
    .from('registrations')
    .select('name, email, country_name, full_number, created_at')
    .order('created_at', { ascending: true })
    .limit(MAX_IN_FILE);

  if (error) return res.status(500).json({ error: 'Erè sèvè.' });

  let csv = 'Non,Email,Peyi,Nimewo,Dat\n';
  for (const r of data) {
    csv += `"${(r.name || '').replace(/"/g, '""')}","${r.email || ''}","${r.country_name || ''}","+${r.full_number}","${r.created_at}"\n`;
  }
